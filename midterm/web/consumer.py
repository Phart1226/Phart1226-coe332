...
# reset database
#response_setDB = requests.get(url="http://localhost:5012/animals/reset")

# grab all animals in db
response_animals = requests.get(url="http://localhost:5012/animals")

# grab all animal uuids
response_keys = requests.get(url="http://localhost:5012/animals/keys")

# grab animal with a specific UUID
response_uuid = requests.get(url="http://localhost:5012/animals/uuid/?uuid=130bb2f6-558b-44b1-afed-c7f4adbddb27")

# update an animals stats given its UUID
response_update = requests.get(url="http://localhost:5012/animals/update/?uuid=130bb2f6-558b-44b1-afed-c7f4adbddb27&head=snake&legs=23&tail=32")


# grab all animals in json file that are within the startDate and endDate
p = {'startDate':'2014-01-01', 'endDate':'2021-01-01'}
response_dates = requests.get(url="http://localhost:5012/animals/dates/", params = p)

# get average legs per animal
response_legs = requests.get(url="http://localhost:5012/animals/legs")

# get count of animals in database
response_count = requests.get(url="http://localhost:5012/animals/count")
